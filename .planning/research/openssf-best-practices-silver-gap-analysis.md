...
| `test_statement_coverage80` | MUST | **MET — measured, not inferred** | The first pass cited the CI *threshold* (85 %). The criterion asks what coverage the suite actually provides, so it was measured with the project's own tooling on 2026-08-27: `busted --coverage spec/` + `luacov` → **415 successes / 0 failures / 0 errors / 0 pending, 93.78 % statement coverage** (1026 hit / 68 missed on `dist/paypal-pos.lua`), confirmed by the CI run on PR #84 rather than by a local run. Worth recording why the figure did **not** move when this pass added six examples: `spec/webbanking_declaration_spec.lua` exercises `src/webbanking_header.lua`, which `.luacov` deliberately excludes, so the new spec buys a real regression guard but zero measured coverage. A first local measurement reported 94.53 % and was wrong — `luacov.stats.out` accumulates across invocations and had absorbed hits from earlier partial runs, including the deliberately mutated one. The clean-checkout CI number is the one cited. Comfortably above the 80 % floor, and above the repo's own 85 % CI gate. |
